    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        ...
        if model_type == "noise":
            ...
            for _ in range(output.ndim-1):
                alpha_t.unsqueeze_(-1)
                sigma_t.unsqueeze_(-1)
            return (x - alpha_t.expand(-1, *x.shape[1:]) * output) / sigma_t.expand(-1, *x.shape[1:]) 
        elif model_type == "v":
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
            for _ in range(output.ndim-1):
                alpha_t.unsqueeze_(-1)
                sigma_t.unsqueeze_(-1)
            return alpha_t.expand(-1, *x.shape[1:]) * output + sigma_t.expand(-1, *x.shape[1:]) * x
        elif model_type == "score":
            sigma_t = noise_schedule.marginal_std(t_continuous)
            for _ in range(output.ndim-1):
                sigma_t.unsqueeze_(-1)
            return -sigma_t.expand(-1, *x.shape[1:])  * output
...
class UniPC:
    def __init__(
        ...
        model_fn,
        noise_schedule,
        algorithm_type:Literal["data_prediction", "noise_prediction"]="data_prediction",
        correcting_x0_fn=None,
        correcting_xt_fn=None,
        thresholding_max_val=1.,
        dynamic_thresholding_ratio=0.995,
        variant:Literal['bh1','bh2','vary_coeff']='bh1'
    ):
        """Construct a UniPC. 
    ...
    def multistep_uni_pc_vary_update(self, x, model_prev_list, t_prev_list, t, order, use_corrector=True):
        ...
        if self.predict_x0:
            ...
            if len(D1s) > 0:
                ...
                for k in range(K - 1):
                    x_t = x_t - alpha_t * h_phi_ks[k + 1] * torch.einsum('bkd,k->bd', D1s, A_p[k])
            # now corrector
            if use_corrector:
                ...
                k = 0
                for k in range(K - 1):
                    x_t = x_t - alpha_t * h_phi_ks[k + 1] * torch.einsum('bkd,k->bd', D1s, A_c[k][:-1])
            ...
            if len(D1s) > 0:
                # compute the residuals for predictor
                for k in range(K - 1):
                    x_t = x_t - sigma_t * h_phi_ks[k + 1] * torch.einsum('bkd,k->bd', D1s, A_p[k])
            # now corrector
            if use_corrector:
                model_t = self.model_fn(x_t, t)
                D1_t = (model_t - model_prev_0)
                ...
                k = 0
                for k in range(K - 1):
                    x_t = x_t - sigma_t * h_phi_ks[k + 1] * torch.einsum('bkd,k->bd', D1s, A_c[k][:-1])
    ...
    def multistep_uni_pc_bh_update(self, x, model_prev_list, t_prev_list, t, order, x_t=None, use_corrector=True):
        ...
        if self.predict_x0:
            ...
            if x_t is None:
                if use_predictor:
                    pred_res = torch.einsum('k,bkd->bd', rhos_p, D1s)
                else:
                    pred_res = 0
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type:Literal['logSNR','time_uniform','time_quadratic']='time_uniform',
        method:Literal['multistep', 'singlestep', 'singlestep_fixed']='multistep', lower_order_final=True, denoise_to_zero=False, return_intermediate=False,
    ):
        """
