...
class FusionNet(nn.Module):
    def __init__(self, resnet_depth:Literal[18, 34, 50, 101, 152],
        ...
        conv_list = [BasicBlock(planes, fuse_chans[0], activation_fnc=activation_func)]
        for i in range(len(fuse_chans)-2):
            conv_list.append(BasicBlock(fuse_chans[i], fuse_chans[i+1], activation_fnc=activation_func))
        conv_list.append(nn.Conv2d(fuse_chans[-2], fuse_chans[-1], kernel_size=1))
