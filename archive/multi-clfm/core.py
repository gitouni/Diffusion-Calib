...
class ResnetEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        features.append(self.encoder.relu(x))  # C = 64
        # self.features.append(self.encoder.layer1(self.encoder.maxpool(self.features[-1])))
        features.append(self.encoder.maxpool(features[-1]))  # C = 64
        features.append(self.encoder.layer1(features[-1]))  # C = 64
        features.append(self.encoder.layer2(features[-1]))  # C = 128
        features.append(self.encoder.layer3(features[-1]))  # C = 256
        features.append(self.encoder.layer4(features[-1]))  # C = 512
        return features
...
class FusionNet(nn.Module):
    def __init__(self, resnet_depth:Literal[18, 34, 50, 101, 152],
                ...
                final_feat_size:Tuple[int,int] = [2,4],
                fusion_knn:int=3,
                fusion_fn:Literal['add','concat','gated','sk']='concat',
                fusion_levels:int=3,
                fuse_chans:List[int] = [512, 256, 128],
                conv2d_ksize:List[int] = [3,3,1],
                activation_func:nn.Module=nn.ReLU(inplace=True)
                ):
        assert len(encoder_3d_chans) == len(pcd_pyramid) 
        assert len(conv2d_ksize) == len(fuse_chans) == fusion_levels
        super().__init__()
        self.fnet_2d = ResnetEncoder(resnet_depth, resnet_pretrained)
        self.fnet_3d = Encoder3D(encoder_3d_chans, pcd_pyramid, norm='batch_norm', embed_norm=embed_norm, k=encoder_3d_knn)
        self.out_dim = final_feat_size[0] * final_feat_size[1] * fuse_chans[-1]
        self.pooling = nn.AdaptiveAvgPool2d(final_feat_size)
        fusion_list = []
        inchan_2d = self.fnet_2d.num_ch_enc[-1]
        inchan_3d = self.fnet_3d.n_chans[-1]
        for i in range(fusion_levels - 1):
            fusion_list.append(CLFM(inchan_2d, inchan_3d, fusion_fn=fusion_fn, fusion_knn = fusion_knn, norm='batch_norm'))
        fusion_list.append(CLFM_2D(inchan_2d, inchan_3d, fusion_fn=fusion_fn, fusion_knn = fusion_knn, norm='batch_norm'))
        self.fusion_list = nn.ModuleList(fusion_list)
        self.fusion_levels = fusion_levels
        planes = inchan_2d
        paddings = [(ksize-1)//2 for ksize in conv2d_ksize]
        conv_list = [nn.Conv2d(planes, fuse_chans[0], kernel_size=conv2d_ksize[0], padding=paddings[0]),
                     activation_func]
        for i in range(len(fuse_chans)-2):
            conv_list.append(nn.Conv2d(fuse_chans[i], fuse_chans[i+1], kernel_size=conv2d_ksize[i+1], padding=paddings[i+1]))
            conv_list.append(activation_func)
        conv_list.append(nn.Conv2d(fuse_chans[-2], fuse_chans[-1], kernel_size=conv2d_ksize[-1], padding=paddings[-1]))
    ...
    def forward(self, image:torch.Tensor, pcd:torch.Tensor, Tcl:torch.Tensor, camera_info:Dict):
        ...
        feat2d, feat3d, xyz = feat_2ds[-1], feat_3ds[-1], xyzs[-1]
        ...
        for i in range(self.fusion_levels - 1):
            feat2d, feat3d = self.fusion_list[i](uv, feat2d, feat3d)  # (B, C, H, W)
        feat2d = self.fusion_list[-1](uv, feat2d, feat3d)
        final_feats = self.conv(feat2d)  # (B, C, h, w)
        return torch.flatten(self.pooling(final_feats), start_dim=1)  # (B, C*h*w)
