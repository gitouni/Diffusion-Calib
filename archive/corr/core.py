...
class FusionNet(nn.Module):
    def __init__(self, resnet_depth:Literal[18, 34, 50, 101, 152],
                ...
                poolings:List[Tuple[int,int]] = [[12,24],[4,8],[2,4],[2,4]],
                fuse_chans:List[int] = [256, 256, 256, 256],
        ...
        for i in range(num_levels):
            ...
            corr_net_list.append(CorrelationNet(**corr_net_argv))
            pooling_list.append(nn.AdaptiveAvgPool2d(poolings[i]))
            mlp_net_list.append(nn.Linear((self.fnet_2d.out_chan[i]+corr_net_argv['planes']) * poolings[i][0] * poolings[i][1], fuse_chans[i]))
        self.clfm_list = nn.ModuleList(clfm_list)
        self.corr_net_list = nn.ModuleList(corr_net_list)
    ...
    def forward(self, image:torch.Tensor, pcd:torch.Tensor, Tcl:torch.Tensor, camera_info:Dict):
        ...
        for i, (feat2d, feat3d, xyz) in enumerate(zip(feat_2ds, feat_3ds, xyzs)):
            ...
            corr_feat = self.corr_net_list[i](feat2d, fused_2d)
            final_feat = self.pooling_list[i](torch.cat([fused_2d, corr_feat], dim=1))  # (B, C, H, W)
            final_feat = self.mlp_net_list[i](torch.flatten(final_feat, start_dim=1))
