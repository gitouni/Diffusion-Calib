...
class FusionNet(nn.Module):
    def __init__(self, resnet_depth:Literal[18, 34, 50, 101, 152],
        ...
        self.fnet_3d = Encoder3D(encoder_3d_chans, pcd_pyramid, norm='batch_norm', embed_norm=False, k=encoder_3d_knn)
        num_levels = len(encoder_2d_chans)
        self.out_plane = sum(encoder_2d_chans) + sum(encoder_3d_chans[-num_levels:])
    ...
    def forward(self, image:torch.Tensor, pcd:torch.Tensor, Tcl:torch.Tensor, camera_info:Dict):
        ...
        return torch.cat(aggregated_feats, dim=1)  # (B, C, h, w)
