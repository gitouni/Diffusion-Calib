...
class Aggregation(nn.Module):
    def __init__(self,inplanes:int, planes=96, mlp_dims:Optional[List[int]]=None, final_feat=(2,4), activation_fn:nn.Module=nn.ReLU(inplace=True)):
        ...
        self.head_conv = nn.Sequential(
            nn.Conv2d(in_channels=inplanes,out_channels=planes*4,kernel_size=3,stride=1,padding=1),
            nn.BatchNorm2d(planes*4),
            nn.Conv2d(in_channels=planes*4,out_channels=planes*4,kernel_size=3,stride=1,padding=1),
            nn.BatchNorm2d(planes*4),
            nn.Conv2d(in_channels=planes*4,out_channels=planes*2,kernel_size=1,stride=1),
            nn.BatchNorm2d(planes*2),
        )
        self.rot_conv = nn.Sequential(
            nn.Conv2d(in_channels=planes*2,out_channels=planes,kernel_size=1,stride=1),
...
class ProjFusionNet(Surrogate, nn.Module):
    def __init__(self, activation:Literal['leakyrelu','relu','elu','gelu'], inplace:bool, encoder_argv:Dict, conv_mlp_argv:Dict) -> None:
        super().__init__()
        activation_func = get_activation_func(activation, inplace)
        self.encoder = FusionNet(**encoder_argv)
        self.conv_mlp = Aggregation(inplanes=self.encoder.out_plane, activation_fn=activation_func, **conv_mlp_argv)

    def forward(self, img:torch.Tensor, pcd:torch.Tensor, Tcl:torch.Tensor, camera_info:Dict):
        feat = self.encoder(img, pcd, Tcl, camera_info)  # (B, C, H, W)
        x0 = self.conv_mlp(feat)
        return x0  # (B, x_dim)
