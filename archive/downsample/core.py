...
class FusionNet(nn.Module):
    def __init__(self, resnet_depth:Literal[18, 34, 50, 101, 152],
                ...
                encoder_2d_chans:List[int]=[32, 64, 128, 256],
                encoder_3d_chans:List[int]=[32, 64, 128, 256],
                ...
                fusion_fn:Literal['add','concat','gated','sk']='sk',
                fusion_knn:int=3,
                ):
        assert len(encoder_2d_chans) == len(encoder_2d_chans) == len(pcd_pyramid)
        super().__init__()
        ...
        for i in range(num_levels):
            clfm_list.append(CLFM_2D(self.fnet_2d.out_chan[i], self.fnet_3d.n_chans[i], fusion_fn=fusion_fn, norm='batch_norm', fusion_knn=fusion_knn))
    ...
    def forward(self, image:torch.Tensor, pcd:torch.Tensor, Tcl:torch.Tensor, camera_info:Dict):
        ...
        for i, (feat2d, feat3d, xyz) in enumerate(zip(feat_2ds, feat_3ds, xyzs[1:])):
