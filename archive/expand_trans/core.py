...
class FusionNet(nn.Module):
    def __init__(self, resnet_depth:Literal[18, 34, 50, 101, 152],
                ...
                expand_trans:List[float] = [-0.015,0,0.015],
                fusion_knn:int=3,
                ):
        ...
        for i in range(num_levels):
            ...
            inplanes = len(expand_trans) ** 3 * self.fnet_2d.out_chan[i] * poolings[i][0] * poolings[i][1]
        ...
        self.expand_trans = []
        for i in expand_trans:
            for j in expand_trans:
                for k in expand_trans:
                    self.expand_trans.append([i,j,k])
    ...
    def forward(self, image:torch.Tensor, pcd:torch.Tensor, Tcl:torch.Tensor, camera_info:Dict):
        ...
        expand_trans = torch.tensor(self.expand_trans).to(pcd)[None,:,:,None]  # (1, K, 3, 1)
        B = pcd.shape[0]
        K = expand_trans.shape[1]
        for i, (feat2d, feat3d, xyz) in enumerate(zip(feat_2ds, feat_3ds, xyzs)):
            xyz_tf = se3_transform(Tcl, xyz).unsqueeze(1)  # (B, 3, N) -> (B, 1, 3, N)
            xyz_tf_expand = xyz_tf + expand_trans # (B, K, 3, N)
            uv = project_pc2image(xyz_tf_expand, camera_info)  # (B, K, 2, N)
            uv[..., 0] *= (feat2d.shape[-1] - 1) / (sensor_w - 1)  # normalize x
            uv[..., 1] *= (feat2d.shape[-2] - 1) / (sensor_h - 1)  # normalize y
            uv = uv.view(B*K, 2, -1)
            feat2d_size = feat2d.shape[1:]  # [C, H, W]
            feat3d_size = feat3d.shape[1:]  # [D, N]
            feat2d_expand = feat2d.unsqueeze(1).expand(-1,K,-1,-1,-1).contiguous().view(B*K,*feat2d_size)
            feat3d_expand = feat3d.unsqueeze(1).expand(-1,K,-1,-1).contiguous().view(B*K,*feat3d_size)
            fused_2d = self.clfm_list[i](uv, feat2d_expand, feat3d_expand)  # (B*K, C, H, W)
            final_feat = self.pooling_list[i](fused_2d)  # (B*K, C, H, W)
            final2d_size = final_feat.shape[1:]
            final_feat = torch.flatten(final_feat.view(B, K, *final2d_size), start_dim=1) # (B,K,C*H*W) -> (B,K*C*H*W)
