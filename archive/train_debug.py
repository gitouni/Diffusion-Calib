...
def main(config:Dict, config_path:str):
    ...
    for epoch_idx in range(start_epoch, run_argv['n_epoch']+1):
        ...
        buffer.put({'weight1':diffuser.x0_fn.model.encoder.fnet_2d.resnet.encoder.conv1.weight.clone(),
                    'weight2':diffuser.x0_fn.model.encoder.fnet_3d.level0_mlp.convs[0].conv_fn.weight.clone()})
        buffer.put({'weight1':diffuser.x0_fn.model.encoder.fnet_2d.resnet.encoder.conv1.weight.clone(),
                    'weight2':diffuser.x0_fn.model.encoder.fnet_3d.level0_mlp.convs[0].conv_fn.weight.clone()})
        with iterator:
            for i, batch in enumerate(train_dataloader):
                ...
                if torch.isnan(loss).sum() > 0:
                    ...
                    iterator.set_postfix(state='nan')
                    iterator.update(1)
                    torch.save(prev_weight,'debug_weight.pt')
                    exit(1)
                loss.backward()
