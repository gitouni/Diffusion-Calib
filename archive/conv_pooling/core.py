...
class resnet18(nn.Module):
    def __init__(self, inplanes=1, planes=32):
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(inplanes, 32, 7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(32),
...
class FusionNet(nn.Module):
    def __init__(self, resnet_depth:Literal[18, 34, 50, 101, 152],
        ...
        assert len(conv2d_ksize) == len(fuse_chans)
        super().__init__()
    ...
    def forward(self, image:torch.Tensor, pcd:torch.Tensor, Tcl:torch.Tensor, camera_info:Dict):
        ...
        final_feats = self.conv(torch.cat(aggregated_feats, dim=1))  # (B, C, h, w)
